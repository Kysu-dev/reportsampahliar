...
export function ReportsDataTable({
    ...
    () => [
      {
        ...
        header: ({ column }) => (
          <button
            ...
            className='inline-flex items-center gap-1 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500 transition hover:text-emerald-700'
          >
            ID Laporan
        ...
        cell: ({ row }) => (
          <p className='font-semibold text-emerald-700'>{row.original.id}</p>
        ),
      },
      {
        id: 'photo',
        ...
        cell: ({ row }) => {
          ...
          if (!thumbnailSource) {
            return (
              <div className='flex h-12 w-20 items-center justify-center rounded-xl border border-dashed border-emerald-200 bg-emerald-50 text-emerald-400'>
          ...
          return (
            <button
              ...
              className='group overflow-hidden rounded-xl border border-emerald-100 ring-offset-2 ring-offset-white transition hover:ring-2 hover:ring-emerald-300'
        ...
        cell: ({ row }) => {
          ...
          return (
            <a
              ...
              target='_blank'
              rel='noreferrer'
              className='inline-flex items-start gap-1.5 text-left text-sm font-medium text-slate-700 transition hover:text-emerald-700'
            >
        ...
        cell: ({ row }) => {
          ...
          return (
            <details className='relative inline-flex'>
              <summary className='inline-flex cursor-pointer list-none items-center gap-1 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700'>
                <MoreHorizontal className='h-3.5 w-3.5' />
                Pilih
                <ChevronDown className='h-3.5 w-3.5' />
              </summary>

              <div className='absolute right-0 top-[110%] z-20 w-44 rounded-xl border border-slate-200 bg-white p-1.5 shadow-lg shadow-slate-900/10'>
                <button
                  onClick={(event) => {
                    closeDropdown(event.currentTarget);
                    openStatusDialog(row.original);
                  }}
                  className='flex w-full items-center gap-2 rounded-xl px-2.5 py-2 text-left text-sm font-medium text-slate-700 transition hover:bg-emerald-50 hover:text-emerald-700'
                >
                ...
                <button
                  ...
                  className='flex w-full items-center gap-2 rounded-xl px-2.5 py-2 text-left text-sm font-medium text-rose-700 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-60'
                >
  ...
  return (
    <>
      <div className='rounded-xl border border-emerald-100 bg-white p-4 shadow-[0_18px_36px_-30px_rgba(8,47,34,0.42)]'>
        <div className='mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between'>
          <div className='relative w-full max-w-md'>
            ...
            <input
              ...
              className='h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-3 text-sm text-slate-900 outline-none transition focus:border-emerald-300 focus:bg-white'
            />
          </div>
        ...
        <div className='mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
          <div className='flex items-center gap-2 text-xs text-slate-500'>
            ...
            <select
              ...
              className='h-8 rounded-xl border border-slate-200 bg-white px-2 text-xs font-medium text-slate-700 outline-none transition focus:border-emerald-300'
            >
              {[5, 8, 10, 20].map((size) => (
          ...
          <div className='flex items-center justify-between gap-3 sm:justify-end'>
            ...
            <div className='flex items-center gap-2'>
              <button
                ...
                className='inline-flex h-9 items-center gap-1 rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-600 transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700 disabled:cursor-not-allowed disabled:opacity-50'
              >
      ...
      >
        <DialogContent className='max-w-3xl rounded-xl border border-slate-200 bg-white' onClose={() => setPreviewImage(null)}>
          <DialogHeader>
      ...
      <Dialog open={statusDialogOpen} onOpenChange={handleStatusDialogChange}>
        <DialogContent className='max-w-xl rounded-xl border border-slate-200 bg-white' onClose={() => handleStatusDialogChange(false)}>
          <DialogHeader>
          ...
          <DialogBody className='space-y-4'>
            <div className='space-y-1.5'>
              <label className='block text-sm font-semibold text-slate-700'>Status Baru</label>
              <select
                value={nextStatus}
                onChange={(event) => setNextStatus(event.target.value as Report['status'])}
                className='h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-medium text-slate-800 outline-none transition focus:border-emerald-300 focus:bg-white'
              >
            ...
            <Textarea
              ...
              className='rounded-xl border-slate-200 bg-slate-50 focus:border-emerald-400 focus:ring-emerald-100'
            />
          </DialogBody>
