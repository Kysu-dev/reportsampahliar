...
import { ReportsDataTable, type DashboardReport } from '@/components/petugas/PetugasReportsDataTable';
import { Toaster_, toast } from '@/components/ui/toaster';
import { authService, reportService } from '@/lib/api';
import type { Report } from '@/types';

type PetugasNavKey = 'overview' | 'reports' | 'history';
...
const navItems: Array<{
  ...
  { key: 'overview', label: 'Ikhtisar Tugas', icon: LayoutDashboard },
  { key: 'reports', label: 'Tindak Lanjut Laporan', icon: ClipboardList },
  { key: 'history', label: 'Riwayat Proses', icon: Truck },
];

const topbarTitle: Record<PetugasNavKey, string> = {
  overview: 'Ikhtisar Tugas Petugas',
  reports: 'Daftar Tindak Lanjut Laporan',
  history: 'Riwayat Proses Laporan',
};
...
interface SidebarContentProps {
  activeNav: PetugasNavKey;
  onNavigate: (next: PetugasNavKey) => void;
  ...
  return (
    <>
      <div className='flex items-center justify-between'>
        ...
        {onCloseMobile ? (
          <button
            ...
            className='inline-flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 transition hover:border-emerald-200 hover:text-emerald-700'
      ...
      <div className='mt-auto space-y-2 border-t border-slate-200 pt-4'>
        <button className='flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-slate-900'>
        ...
        <button
          ...
          className='flex w-full items-center gap-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-3 text-left text-sm font-semibold text-rose-700 transition hover:bg-rose-100'
        >
...
export function PetugasDashboard() {
  ...
  const [activeNav, setActiveNav] = useState<PetugasNavKey>('reports');
  ...
  const handleLogout = useCallback(async () => {
    ...
    toast.info('Sesi petugas telah diakhiri');
    ...
    <>
      <main className='min-h-screen bg-[#F8FAFC] text-slate-900'>
        <div className='relative flex min-h-screen'>
          <aside className='sticky top-0 hidden h-screen w-72 shrink-0 border-r border-slate-200 bg-white/95 p-4 lg:flex lg:flex-col'>
          ...
          {mobileSidebarOpen ? (
            <>
              ...
              <aside className='fixed inset-y-0 left-0 z-50 w-[82%] max-w-xs border-r border-slate-200 bg-white p-4 lg:hidden'>
                <SidebarContent
                  activeNav={activeNav}
          ...
          <div className='flex min-w-0 flex-1 flex-col'>
              ...
              <div className='flex h-20 items-center justify-between px-4 sm:px-6 lg:px-8'>
                <div className='flex items-center gap-3'>
                  <button
                    ...
                    className='inline-flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-700 transition hover:border-emerald-200 hover:text-emerald-700 lg:hidden'
                  ...
                  <div>
                    <p className='text-[11px] font-semibold uppercase tracking-[0.26em] text-emerald-700'>Panel Petugas</p>
                    <h1 className='text-xl font-black text-slate-900'>{topbarTitle[activeNav]}</h1>
                  </div>
                </div>

                <div className='flex items-center gap-2 sm:gap-4'>
                  <button className='relative inline-flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-700 transition hover:border-emerald-200 hover:text-emerald-700'>
                    <Bell className='h-4 w-4' />
                    <span className='absolute right-2 top-2 h-2 w-2 rounded-full bg-emerald-500' />
                  </button>

                  <div className='hidden text-right sm:block'>
                    <p className='text-sm font-semibold text-slate-900'>Petugas CleanTrack</p>
                    <p className='text-xs text-slate-500'>Tim Operasional</p>
                  </div>
            ...
            <section className='flex-1 space-y-6 px-4 py-6 sm:px-6 lg:px-8'>
              {showReportsModule ? (
                <>
                  <div className='grid gap-4 sm:grid-cols-2 xl:grid-cols-4'>
                    {summaryCards.map((card) => {
                      ...
                      return (
                        <article
                          key={card.key}
                          className='rounded-xl border border-slate-200 bg-white p-5 shadow-[0_20px_36px_-30px_rgba(11,28,48,0.45)]'
                        >
                          ...
                          <div className='mt-4 rounded-xl bg-slate-50 px-2 py-1'>
                  ...
                  <div className='space-y-3'>
                    <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
                      <div>
                        <h2 className='text-2xl font-black text-slate-900'>Daftar Laporan</h2>
                      ...
                      <button
                        ...
                        className='inline-flex h-10 items-center justify-center rounded-xl border border-emerald-200 bg-emerald-50 px-4 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-100'
                      >
                        Refresh Data
                      </button>
                    </div>
                ...
                <div className='rounded-xl border border-slate-200 bg-white p-8 shadow-[0_20px_36px_-30px_rgba(11,28,48,0.45)]'>
                  <h2 className='text-2xl font-black text-slate-900'>{topbarTitle[activeNav]}</h2>
                  <p className='mt-2 max-w-2xl text-sm text-slate-500'>
                    Modul ini sedang dipersiapkan. Sementara itu, gunakan menu Tindak Lanjut Laporan untuk workflow utama petugas.
                  </p>
                </div>
              )}
            </section>
